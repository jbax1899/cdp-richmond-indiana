...
1.  Create the GitHub repository for this deployment to live in.
    ...
    -   Set the repo name to: **cdp-richmond-indiana**
    -   Set the repo owner to: **jbax1899**
...
1.  Initialize the basic project infrastructure.
    ...
    ```bash
    just init cdp-richmond-(in)-jvrzndvq
    ```
...
1.  Create (or re-use) a
    [Google Cloud billing account](https://console.cloud.google.com/billing/linkedaccount?project=cdp-richmond-(in)-jvrzndvq)
    and attach it to the newly created project (cdp-richmond-(in)-jvrzndvq).
...
1.  Create a GitHub Personal Access Token.
    ...
    -   Give the token a descriptive name / note. We recommend: `cdp-richmond-(in)-jvrzndvq`
...
1.  Attach the Google Service Account JSON as GitHub Repository Secret.
    ...
    1. Create a [new secret](https://github.com/jbax1899/cdp-richmond-indiana/settings/secrets/actions/new)

    -   Set the name to: **GOOGLE_CREDENTIALS**
    -   Set the value to: the contents of the file `.keys/cdp-richmond-(in)-jvrzndvq.json`
    -   Click "Add secret"

    2. Create a [new secret](https://github.com/jbax1899/cdp-richmond-indiana/settings/secrets/actions/new)

    -   Set the name to: **FIREBASE_TOKEN**
    -   Set the value to: the value of the Firebase CI token you created in a prior step.
    -   Click "Add secret"

    3. Create a [new secret](https://github.com/jbax1899/cdp-richmond-indiana/settings/secrets/actions/new)
...
1.  Build the basic project infrastructure.
    ...
    ```bash
    just setup cdp-richmond-(in)-jvrzndvq us-east1
    ```

1.  Initialize Firebase Storage.

    [Firestore Storage Page](https://console.firebase.google.com/u/0/project/cdp-richmond-(in)-jvrzndvq/storage)
...
1.  Initialize and push the local repository to GitHub.
    ...
    ```bash
    git remote add origin https://github.com/jbax1899/cdp-richmond-indiana.git
    ```

    Or (with SSH):

    ```bash
    git remote add origin git@github.com:jbax1899/cdp-richmond-indiana.git
    ```
...
1.  Once the
    ["Web App" GitHub Action Successfully Complete](https://github.com/jbax1899/cdp-richmond-indiana/actions?query=workflow%3A%22Web+App%22)
    configure GitHub Pages.

    Go to your repository's [GitHub Pages Configuration](https://github.com/jbax1899/cdp-richmond-indiana/settings/pages)
...
Navigate and follow the instructions in the the file: `python/cdp_richmond_(in)_backend/scraper.py`.
